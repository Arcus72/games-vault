...
export const FULL_FILTERS: FilterSection[] = [
  ...
  { type: "range", label: "Year span", name: "yearSpan", open: true },
  {
    type: "checkboxes",
    label: "Tags",
    name: "tags",
    options: ["RPG", "Adventure", "Indie", "Action"],
    checked: ["RPG"],
    open: true,
  },
  {
  ...
  if (s.type === "search") {
    return (
      <input
        ...
        placeholder={s.placeholder ?? "Search by name"}
    ...
    <details className="filters__section" open={s.open}>
      ...
      {s.type === "range" && range && (
        <div className="filters__year">
          <input
            ...
            placeholder={s.startLabel ?? "Start"}
            value={range.start ?? ""}
      ...
      {s.type === "checkboxes" && checked && (
        <div className="filters__tags">
          ...
          {/* <input className="filters__search" placeholder="Search by name" /> */}
        </div>
      )}
    </details>
  );
